...
class _NotificationDrawerState extends State<NotificationDrawer> {
  ...
  _setupNotiData() async{
    List<NotificationClass> data=await DatabaseServiceNotification.getNotification();
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                Text("Thông Báo",style: TextStyle(color: Colors.white,fontSize: 20)),
                IconButton(onPressed: () {
                  Scaffold.of(context).closeEndDrawer();
                }, icon: Icon(Icons.close),color: Colors.white,iconSize: 28,)
              ],
            ),
            ...
            children: [
              Row(
                ...
                children: [
                  Container(
                    width: 150,
                    child: Text(notifi.title,style: TextStyle(fontWeight: FontWeight.bold,fontSize: 18),),
                  ),
                  Text(DateFormat.yMMMd().format(notifi.time,),style: TextStyle(fontSize: 15), )
                ],
              
              ),
              Padding(padding: EdgeInsets.only(top: 8)),
              Text(notifi.detail,style: TextStyle(fontSize: 15),)
            ],
          )
        )
      )
